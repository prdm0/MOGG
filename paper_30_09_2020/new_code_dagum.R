...
# integrate(f = pdf_dagum, lower = 0, upper = Inf, alpha = 1.2,
#           beta = 1.6, p = 2.2)
...
rmogdagum <- function(n = 1L, theta, a, alpha, beta, p) {
  ...
  c <-
    pdf_mogdagum(x_max, theta, a, alpha, beta, p) / pdf_dagum(x_max,
                                                              alpha = alpha,
                                                              beta = beta,
                                                              p = p)
...
simulate <- function(n) {
  ...
  eval(parse(
    text = glue(
      "bias_{n} <- apply(X = result_mc, MARGIN = 1L, FUN = bias_function,
                         par_true = true_parameters) %>%
                   apply(MARGIN = 1L, FUN = mean)"
    )
  ))
  eval(parse(text = glue(
    "save(file = \"bias_{n}.RData\", bias_{n})"
  )))
  
  # Mean Square Error -------------------------------------------------------
  eval(parse(
    text = glue(
      "mse_{n} <- apply(X = result_mc, MARGIN = 1L, FUN = mse_function,
                       par_true = true_parameters) %>%
                  apply(MARGIN = 1L, FUN = mean)"
    )
  ))
  eval(parse(text = glue(
    "save(file = \"mse_{n}.RData\", mse_{n})"
  )))
  
  # Total Time --------------------------------------------------------------
  eval(parse(text = glue("time_{n} <- total_time")))
  eval(parse(text = glue(
    "save(file = \"time_{n}.RData\", time_{n})"
  )))
  
  # Result MC
  eval(parse(text = glue("result_{n} <- result_mc")))
  eval(parse(text = glue(
    "save(file = \"result_{n}.RData\", result_{n})"
  )))
...
latex <-
  print.xtable(
    xtable(tabela,
           caption = "Mean bias of EMV obtained by the BFGS method in 10,000
